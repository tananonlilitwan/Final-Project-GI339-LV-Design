...
public class MaskPacker : EditorWindow
{
    ...
    {
        if(inputMetallic != null)
        {
            return AssetDatabase.GetAssetPath(inputMetallic);
        }
        else if (inputAmbientOcclusion != null)
        {
            return AssetDatabase.GetAssetPath(inputAmbientOcclusion);
        }
        else if (inputDetailMask != null)
        {
            return AssetDatabase.GetAssetPath(inputDetailMask);
        }
        else if (inputSmooth_Rough != null)
        {
            return AssetDatabase.GetAssetPath(inputSmooth_Rough);
        }
    ...
    {
        string assetPath = Path.GetDirectoryName(GetAssetPath());
        string texPath = assetPath.Or("Assets");

        var path = EditorUtility.SaveFilePanelInProject("Save Texture To Directory", "_MaskMap", "png", "Saved", texPath);
