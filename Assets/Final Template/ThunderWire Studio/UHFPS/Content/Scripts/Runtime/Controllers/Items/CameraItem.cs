...
namespace UHFPS.Runtime
{
    public class CameraItem : PlayerItemBehaviour
    {
        ...
        public Percentage ExposureDrainStart = 50;
        public float LowNVExposure = 0f;
        public bool NormalModeDrainBattery = false;
        ...
        private void Awake()
        {
            ...
            if (NVComponent.TryGetVolumeComponent(out ColorAdjustments colorAdjustments))
            {
        ...
        private void UpdateBattery()
        {
            ...
            if(NVComponent.TryGetVolumeComponent(out ColorAdjustments colorAdjustments))
            {
                float exposureT = Mathf.InverseLerp(0, ExposureDrainStart.Ratio(), batteryEnergy);
                float postExposure = Mathf.Lerp(LowNVExposure, defaultExposure, exposureT);
        ...
        private void SetNVState(bool state, bool isInitial = false)
        {
            ...
            NVComponent.SetVolumeComponentActive(state);
            // NV Enabled = HighBatteryDrainSpeed, NV Disabled = NormalModeDrainBattery (LowBatteryDrainSpeed) / 0
            drainSpeed = state ? HighBatteryDrainSpeed : NormalModeDrainBattery ? LowBatteryDrainSpeed : 0;
