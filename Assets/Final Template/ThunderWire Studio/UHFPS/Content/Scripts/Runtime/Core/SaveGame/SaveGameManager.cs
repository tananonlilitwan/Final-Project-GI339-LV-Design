    ...
    public class SaveGameManager : Singleton<SaveGameManager>
    {
        ...
        private void Start()
        {
            ...
            if (GameLoadType == LoadType.LoadPlayer || shouldLoadPlayer)
            {
                ...
                else if (Debugging) Debug.Log($"[SaveGameManager] Player data cannot be loaded because it is null.");
        ...
        /// <summary>
        /// Set the load type to load the last game state or player state, depending on whether the game state exists.
        /// <br>TThe game will be loaded from the last scene save or only the player data will be loaded.</br>
        /// </summary>
        /// <param name="sceneName">The name of the scene to be loaded.</param>
        /// <param name="folderName">The name of the saved game folder.</param>
        public static void SetLoadGameOrPlayerState(string sceneName, string folderName)
        {
            bool gameStateExist = true;
            if (string.IsNullOrEmpty(folderName) && LastSceneSaves != null)
            {
                if (gameStateExist = LastSceneSaves.TryGetValue(sceneName, out var lastSceneSave))
                    folderName = lastSceneSave;
            }

            if (gameStateExist) SetLoadGameState(sceneName, folderName);
            else SetLoadPlayerData(sceneName);
        }
        ...
        {
            if(ScreenshotVolume != null && ScreenshotVolume.customPasses[0] is ScreenshotPass screenshotPass)
